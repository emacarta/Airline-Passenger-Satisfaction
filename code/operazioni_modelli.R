...
col_numeriche <- scale(data[,c(2,5,18,19)])
...
col_numeriche_test <- scale(data_test[,c(2,5,18,19)])
...
data_test$Age <- col_numeriche_test$Age
...
LogiRegModel2 <- glm( satisfaction ~ ., data = df_train, family = "binomial")

summary(LogiRegModel2)

prob <- predict(LogiRegModel2,df_test, type = "response")
pred <- ifelse(prob > 0.5, 1, 0)
